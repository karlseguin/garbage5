...
func (_ ListTests) PushesNewItemToFront() {
	list := NewList()
	cr1, cr2, cr3 := &Entry{id: 2}, &Entry{id: 1}, &Entry{id: 3}
...
func (_ ListTests) RemovesItemFromTheList() {
	list := NewList()
	cr1, cr2, cr3, cr4, cr5 := &Entry{id: 2}, &Entry{id: 1}, &Entry{id: 3}, &Entry{id: 4}, &Entry{id: 5}
	list.PushToFront(cr2)
	list.PushToFront(cr3)
...
func (_ ListTests) NoopOnRemovingNoExistingItem() {
	list := NewList()
	list.Remove(&Entry{id: 994})
	assertList(list)
}

func assertList(list *List, items ...*Entry) {
	Expect(list.head.id).To.Equal(uint32(0))
	Expect(list.tail.id).To.Equal(uint32(0))
	node := list.head
	for _, item := range items {
		node = node.next
		Expect(item).To.Equal(node)
	}
	Expect(node.next.id).To.Equal(uint32(0))
}
